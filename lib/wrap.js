...
Wrap.prototype.bundle = function () {
    ...
    var src = []
        .concat('(function(__require, __module) {')
...
function makeTarget (file, root) {
    ...
    else if (Wrap.builtins[file] || !file.match(/^\//)) {
...
Wrap.prototype.require = function (mfile, opts) {
    ...
    if (pkg.embedify && pkg.embedify.require) {
        required.strings = required.strings.concat(
            pkg.embedify.require
        );
    }
    ...
    nub(required.strings)
        .filter(function (req) { return req.match(/^(\.\.?)?\//) || Wrap.builtins[req] })
        .forEach(function (req) { self.require(req, { basedir : opts.basedir, root : opts.root }) });
...
Wrap.prototype.embed = function (mfile, opts) {
    this.require(mfile, opts);

    this.addEntry('entry.js', {
        body: "__module.exports = require('" + mfile + "');"
    });
